...
这篇笔记的主线是：语言模型一开始只看固定长度窗口；RNN 引入隐藏状态，让模型能处理任意长度序列；GRU/LSTM 和 Attention 则是在修补 RNN 长距离记忆与信息瓶颈的问题。
...
flowchart TB
    ...
    classDef decision fill:#fff7ed,stroke:#f59e0b,color:#7c2d12,stroke-width:1.5px;
    classDef output fill:#f5f3ff,stroke:#8b5cf6,color:#3b0764,stroke-width:1.5px;
    A["完整链式法则"] --> A1["当前词依赖所有历史词"]
    B["n-gram 近似"] --> B1["当前词只依赖最近 n 个词"]
    A1 --> C["表达强，但数据稀疏"]
    B1 --> D["容易估计，但看不见远距离依赖"]
...
可以把 n-gram 看成是在历史序列上开了一个固定长度的观察窗。窗口外的信息被直接丢掉。
...
- 核心架构公式 (Equation 4)：$$\hat{y}=softmax(W^{(2)}tanh(W^{(1)}x+b^{(1)})+W^{(3)}x+b^{(3)})$$
    ...
    这一步仍然是固定窗口模型：输入长度固定，窗口外的信息没有进入网络。
...
##### 视觉脚手架：RNN 展开后像一排共享参数的同构单元

![RNN 时间展开：同一套参数在每个时间步重复使用](<pic/whiteboard_exported_image (8).png>)
...
flowchart LR
    ...
    class X1,H0,X2,X3 source;
    class H1,H2,H3 process;
    class Y1,Y2,Y3 output;
```

图上每个时间步看起来是不同盒子，但里面用的是同一套 $W^{(hh)}$、$W^{(hx)}$、$W^{(S)}$。这就是“参数共享”。
...
普通前馈网络只从“上一层”接误差；RNN 在时间展开后，$h_t$ 还会从 $h_{t+1}$ 接误差。这就是 BPTT 比普通 backprop 更绕的地方。
...
梯度 = 局部因子(t) x 局部因子(t-1) x ... x 局部因子(k+1)

若每个因子约 0.8，传 20 步后约 0.8^20
若每个因子约 1.2，传 20 步后约 1.2^20
```

所以问题不在“乘了一次”，而在同类因子沿时间轴重复相乘很多次。小于 1 会消失，大于 1 会爆炸。
...
##### 视觉脚手架：GRU 用两个门混合旧记忆与新候选

![GRU 内部结构：更新门与重置门如何混合旧记忆和新信息](<pic/whiteboard_exported_image (9).png>)
...
更新门 $z_t$ 是理解 GRU 的中心：它越接近 1，越偏向保留旧记忆；越接近 0，越偏向写入新候选。
...
![LSTM 内部结构：cell state highway 与三大门控](<pic/whiteboard_exported_image (10).png>)
...
flowchart LR
    ...
    classDef decision fill:#fff7ed,stroke:#f59e0b,color:#7c2d12,stroke-width:1.5px;
    classDef output fill:#f5f3ff,stroke:#8b5cf6,color:#3b0764,stroke-width:1.5px;
    C0["长期记忆 c_t-1"] --> F["遗忘门 f_t"]
    X["当前输入 x_t"] --> I["输入门 i_t"]
    H0["短期展示 h_t-1"] --> I
    X --> G["候选记忆 c_tilde"]
    H0 --> G
    F --> C["新的长期记忆 c_t"]
    ...
    class C0,X,H0 source;
    class F,I,G,C,O process;
    class H output;
```

和 GRU 相比，LSTM 把“长期保存的内容”放在 $c_t$，把“当前对外暴露的内容”放在 $h_t$。复习时先分清这两条线，门的作用会清楚很多。
...
#### 视觉脚手架：Seq2Seq 是 Encoder 到 Decoder 的接力
...
没有 Attention 的 Seq2Seq 最大瓶颈就在中间的“句子编码向量”：整句源语言信息都要被压进一个固定长度表示。
...
#### 视觉脚手架：Attention 是一次软查询

![Seq2Seq 加 Attention 的数据流：Decoder 提供 Query，Encoder 提供 Keys 和 Values](<pic/whiteboard_exported_image (11).png>)
...
flowchart LR
    ...
    classDef decision fill:#fff7ed,stroke:#f59e0b,color:#7c2d12,stroke-width:1.5px;
    classDef output fill:#f5f3ff,stroke:#8b5cf6,color:#3b0764,stroke-width:1.5px;
    Q["Decoder 当前状态 Query"] --> S1["和每个 Key 打分"]
    K["Encoder hidden states 作为 Keys"] --> S1
    S1 --> A["softmax 得到权重"]
    A --> W["加权求和"]
    V["Encoder hidden states 作为 Values"] --> W
    W --> O["attention 输出"]

    class Q,K,V source;
    class S1,A,W process;
    class O output;
```

如果把 Encoder hidden states 想成一张“可查询的记忆表”，那么 Query 不是精确取某一行，而是给每一行一个权重，再做加权平均。
