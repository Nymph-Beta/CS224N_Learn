...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # [nn.Embedding 写法] 如果允许使用 nn.Embedding，上面这行可替换为:
        #   self.embeddings = nn.Embedding.from_pretrained(torch.tensor(embeddings), freeze=False)
        # nn.Embedding 内部做的事情和我们手动实现的一样：存一个权重矩阵，用索引查行。
        # from_pretrained 用预训练向量初始化，freeze=False 表示训练时可更新。
        ...
        # [nn.Linear 写法] 上面 W 和 b1 可替换为:
        #   self.embed_to_hidden = nn.Linear(self.n_features * self.embed_size, self.hidden_size)
        # nn.Linear 内部自动创建 weight (形状为 out×in) 和 bias，默认用 kaiming_uniform 初始化。
        # 注意: nn.Linear 的 weight 形状是 (out_features, in_features)，与我们手动定义的 (in, out) 转置。
        #       所以 forward 中用法也不同: nn.Linear 用 self.embed_to_hidden(x)，内部计算 x @ W^T + b。
    ...
    def embedding_lookup(self, w):
        ...
        # 对应了公式里的 x = [E_{w1}, ..., E_{wm}]。
        # 不能用 nn.Embedding，但可以直接用张量索引（self.embeddings[w]）来实现。

        # 用 w 作为索引从 self.embeddings 里取出对应行 → 得到 (batch_size, n_features, embed_size)
        x = self.embeddings[w]

        # 把后两维拼成一维 → 得到 (batch_size, n_features * embed_size)
        x = x.view(w.shape[0], -1)

        # [nn.Embedding 写法] 如果 __init__ 中用了 nn.Embedding，这里只需:
        #   x = self.embeddings(w)          # (batch_size, n_features, embed_size)
        #   x = x.view(w.shape[0], -1)      # (batch_size, n_features * embed_size)
        # nn.Embedding 的 forward 本质就是用索引查权重矩阵的行，和我们写的 self.embeddings[w] 一样。
    ...
    def forward(self, w):
        ...
        # 对应了公式里的 h = ReLU(xW + b1)
        # 矩阵乘法用 torch.matmul，ReLU 用 F.relu

        # 调用 embedding_lookup 得到 x
        x = self.embedding_lookup(w)

        # 计算 h = ReLU(xW + b1)，然后对 h 做 dropout
        h = F.relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        h = self.dropout(h)

        # 计算 logits = hU + b2 （不需要 softmax，因为 CrossEntropyLoss 内部会做）
        logits = torch.matmul(h, self.hidden_to_logits_weight) + self.hidden_to_logits_bias

        # [nn.Linear 写法] 如果 __init__ 中用了 nn.Linear，上面可简化为:
        #   h = F.relu(self.embed_to_hidden(x))  # nn.Linear 内部计算 x @ W^T + b
        #   h = self.dropout(h)
        #   logits = self.hidden_to_logits(h)
        # nn.Linear 封装了矩阵乘法和加偏置，且自动处理了 weight 的转置。
